...
ROOT = Path(".")
INPUT_DIR = ROOT / "data" / "multi_state_lgd_input"
OUT_DIR = ROOT / "data"
PKG_DATA = ROOT / "indic_places" / "data"

CUSTOM_PLACES = PKG_DATA / "custom_places.txt"
OUT_UNIQUE = OUT_DIR / "multi_state_lgd_names_unique.txt"
OUT_FULL = OUT_DIR / "multi_state_lgd_names_full.csv"
OUT_FULL_GZ = OUT_DIR / "multi_state_lgd_names_full.csv.gz"
...
def parse_states(value: str | None) -> set[str]:
    if not value:
        values = DEFAULT_STATES
    else:
        values = [x.strip() for x in value.split(",") if x.strip()]
    return {name_key(x) for x in values}
...
def looks_like_header(values: list[str]) -> bool:
    ...
    has_name_col = any(
        any(normalize_col(hint) in c for hint in NAME_HINTS)
        for c in cols
    )
    has_context_col = any(
...
def row_state_allowed(row: dict, headers: list[str], allowed_state_keys: set[str]) -> tuple[bool, str]:
    state_i = find_index(headers, STATE_HINTS)
    if state_i is None:
        # Many state-specific LGD files do not carry state columns.
        return True, ""

    state = clean_cell(row.get(headers[state_i], ""))
    return name_key(state) in allowed_state_keys, state.upper()
...
def extract_records(source_name: str, row: dict, allowed_state_keys: set[str]) -> list[dict]:
    headers = list(row.keys())

    allowed, state_from_row = row_state_allowed(row, headers, allowed_state_keys)
    if not allowed:
        return []
...
def update_pyproject(version: str) -> None:
    ...
    text = re.sub(
        r'(?m)^description\s*=.*$',
        'description = "Indian place-name lookup, OCR address cleanup, multi-state LGD locality vocabulary, correction, extraction, and address intelligence"',
        text,
    )
    PYPROJECT.write_text(text, encoding="utf-8")


def update_readme(states_text: str) -> None:
...
"""
    ...
    if "### Multi-state LGD locality vocabulary" not in text:
        README.write_text(text.rstrip() + "\n\n" + note.strip() + "\n", encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--states", default=",".join(DEFAULT_STATES), help="Comma-separated states to import")
    ...
    allowed_state_keys = parse_states(args.states)
    states_text = ", ".join([x.strip().upper() for x in args.states.split(",") if x.strip()])

    records = []
    seen_full = set()

    for source_name, row in iter_all_input_rows():
        for rec in extract_records(source_name, row, allowed_state_keys):
            full_key = (
                name_key(rec["name"]),
                name_key(rec["state"]),
    ...
    print("=" * 80)
    print("SUMMARY")
    print(f"States requested              : {states_text}")
